...
var dog_key: String = ""
var dog_name: String = ""
var dispenser_type: String = "basic"
var capacity: float = 100.0
var fullness: float = 100.0

const PLAYER_DATA_STORAGE = preload("res://storage/player_data.gd")
const DISPENSER_PLACEHOLDER_TEX = preload("res://sprites/decoration/water_dispenser.jpg")
...
func _load_stats() -> void:
	if dog_key == "":
		return
	var data = PLAYER_DATA_STORAGE.get_dog_dispenser(dog_key)
	dispenser_type = data.get("type", "basic")
...
func _update_visuals() -> void:
	if label:
		...
		label.text = "%s's Dispenser\n[%.0f / %.0f]" % [dog_name, fullness, capacity]
		label.modulate = Color.CYAN if pct >= 0.6 else (Color.YELLOW if pct >= 0.25 else Color.RED)
	
	if sprite:
		# Tint dispenser based on type
		match dispenser_type:
			"silver":
				sprite.modulate = Color(0.8, 0.9, 1.0, 1.0) # Silver
			"gold":
...
func refill() -> void:
	...
	fullness = capacity
	save_stats()
	_show_floating_text("Refilled!")
	
	# Update HUD/Stats Panel if it's currently open
	var main_room = get_tree().current_scene
	if main_room and main_room.has_method("refresh_food_hud"):
		main_room.refresh_food_hud()

func drink_water(amount: float) -> float:
	"""Called by the dog. Returns the actual amount of water drunk."""
	var to_drink = min(amount, fullness)
	fullness -= to_drink
	save_stats()
	return to_drink
