...
func add_item_to_grid(grid: GridContainer, texture_path: String) -> void:
	...
	# Calculate dynamic width based on aspect ratio, fixed height of 16
	var display_height := 16
	var display_width := int((float(frame_width) / float(frame_height)) * display_height)
