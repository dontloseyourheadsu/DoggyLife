...
func _process(_delta: float) -> void:
	...
	if camera and animated_dog_sprite:
		animated_dog_sprite.global_rotation = camera.global_rotation
...
func _physics_process(delta: float) -> void:
	...
	if _command_active:
		var target_xz = Vector3(_command_target.x, global_position.y, _command_target.z)
		var to_target := target_xz - global_position
...
func _start_walking_away_from_collision():
	...
	if get_slide_collision_count() > 0:
		...
		var normal_xz = Vector3(normal.x, 0, normal.z)
